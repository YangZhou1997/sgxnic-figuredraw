...
def get_type(ori_name):
    switcher = {
        **dict.fromkeys([f"./{raw_data_dir}/nic"], "SmartNIC"),
        **dict.fromkeys([f"./{raw_data_dir}/nb"], "NetBricks"),
        **dict.fromkeys([f"./{raw_data_dir}/sb"], "SafeBricks"),
    }
    ...
    for f_name in f_list:
        with open(f_name, 'r') as f:
            ...
            while raw_entry:
                ...
                _t = float(entry_array[3])/norm_base
                _avg_l = float(entry_array[4])/norm_base
                _tail_l = float(entry_array[5])/norm_base
...
def draw_t_bar_for_core_ipsec_trace(_core, _ipsec, _trace):
    N = len(all_tasks)
    ...
    plt.tight_layout()
    plt.savefig('./figures/bar_norm/throughput/t_bar_%scores_%s_trace_%s.pdf' % (_core, _ipsec, _trace))
    plt.clf()
...
def draw_l_bar_for_core_ipsec_trace(_core, _ipsec, _trace):
    N = len(all_tasks)
    ind = np.arange(N) * 10 + 10    # the x locations for the groups
    width = 6.0/len(all_types)       # the width of the bars: can also be len(x) sequence
    ...
    legends = list()
    for _type in all_types:
        data_vec_avg, data_vec_tail = get_l_draw_data_vary_task(_type, _ipsec, _trace, _core)
    ...
    plt.tight_layout()
    plt.savefig('./figures/bar_norm/latency/l_bar_%scores_%s_trace_%s.pdf' % (_core, _ipsec, _trace))
    plt.clf()
...
def draw_t_bar_for_core_ipsec_16_1(_ipsec):
    ...
    plt.tight_layout()
    plt.savefig('./figures/bar_norm/nic16-nb1/t_bar_16_1cores_%s.pdf' % (_ipsec,))
    plt.clf()
...
def draw_l_bar_for_core_ipsec_16_1(_ipsec):
    ...
    plt.tight_layout()
    plt.savefig('./figures/bar_norm/nic16-nb1/l_bar_16_1cores_%s.pdf' % (_ipsec,))
    plt.clf()
...
def draw_t_bar_for_core_ipsec_1811(_ipsec, _trace):
    ...
    plt.tight_layout()
    plt.savefig('./figures/bar_norm/nic1_8-nb1/t_bar_1811_%s_cores_%s.pdf' % (_trace, _ipsec))
    plt.clf()
...
def draw_l_bar_for_core_ipsec_1811(_ipsec, _trace):
    ...
    for label in plt.axes().xaxis.get_majorticklabels():
        label.set_transform(label.get_transform() + offset)
    plt.tight_layout()
    plt.savefig('./figures/bar_norm/nic1_8-nb1/l_bar_1811_%s_cores_%s.pdf' % (_trace, _ipsec))
    plt.clf()
...
def draw_t_bar_for_core_ipsec_811(_ipsec, _trace, _nic_core_num):
    ...
    plt.tight_layout()
    plt.savefig('./figures/bar_norm/nic8-nb1/t_bar_%s11_%s_cores_%s.pdf' % (_nic_core_num, _trace, _ipsec))
    plt.clf()
...
def draw_l_bar_for_core_ipsec_811(_ipsec, _trace, _nic_core_num):
    ...
    for label in plt.axes().xaxis.get_majorticklabels():
        label.set_transform(label.get_transform() + offset)
    plt.tight_layout()
    plt.savefig('./figures/bar_norm/nic8-nb1/l_bar_%s11_%s_cores_%s.pdf' % (_nic_core_num, _trace, _ipsec))
    plt.clf()
...
if __name__ == '__main__':
    ...
    data_load(f"./{raw_data_dir}/nic")
    data_load(f"./{raw_data_dir}/nb")
    data_load(f"./{raw_data_dir}/sb")
    ...
    # draw_t_bar_for_core_ipsec_811("gcm_ipsec", "64B", "1")
    # draw_t_bar_for_core_ipsec_811("gcm_ipsec", "ICTF", "1")
    # draw_t_bar_for_core_ipsec_811("no_ipsec", "64B", "1")
    # draw_t_bar_for_core_ipsec_811("no_ipsec", "ICTF", "1")
